...
class OutputWriter:
    ...
    def create_kyc_table(self, doc: Document, kyc_profiles_output: dict):
        ...
        for i, (key, value) in enumerate(reference_checks.items()):
            ...
            # Display name — look up from constants, fall back to key name
            ...
            # Status color
            check_status = (
                value.get("status", False) if isinstance(value, dict) else False
            )
            colour = "#29B95C" if check_status else "#DC3939"
            reason_text = (
            ...
            if key == "consistency_checks_within_kyc_contradiction_checks":
                ...
                cell = row_cells[2]
                cell.add_paragraph(
                    "Please refer to the annex for LLM analysis details.\n"
                )
                for partner_name in partner_names:
                    partner_value = kyc_profiles_output[partner_name].get(key, {})
                    raw_checks = (
                        partner_value.get("raw_checks", {})
                    ...
                    checks = [
                        {
                            ...
                            "outcome": (
                                "Objections"
                                if v.get("contradictory", False)
                                else "No Objections"
                            ),
                        }
                        for k, v in raw_checks.items()
                    ]
                    logger.info(f"Section 11.2 - Partner: {partner_name}")
                    logger.info(f"Section 11.2 - raw_checks: {raw_checks}")
                    logger.info(f"Section 11.2 - checks to render: {checks}")
                    cell.add_paragraph().add_run(
                        prettify_name(partner_name)
                    ).bold = True
                    if checks:
                        self.add_subtable(cell, checks)
            elif key == "siap_flags":
                cell = row_cells[2]
                for partner_name, raw_data in value["raw_data"].items():
                    summary = (
                        raw_data[-1]
                        if raw_data is not None
                        else "No SIAP activities detected."
                    )

                    para = cell.add_paragraph()
                    para.add_run(f"{prettify_name(partner_name)}\n").bold = True
                    para.add_run(f"Results summary: {summary}\n")
                    if raw_data is None:
                        continue
                    self.add_subtable(cell, raw_data[:-2], apply_title_transform=False)
                    self.add_subtable(
                        cell,
                        [
                            {"Activity": idx, "Description": sow["sow"]}
                            for idx, sow in enumerate(raw_data[-2], 1)
                        ],
                        apply_title_transform=False,
    ...
    def create_table(
        ...
        print(
            f"create_table called: is_edd={is_edd}, num_keys={len(processed_profile_output)}"
        )
        cols = 2
        doc.add_paragraph()
        skip_keys = {
        ...
        for key, value in processed_profile_output.items():
            ...
            if key not in ["activity", "total_wealth_composition"]:
                ...
                cell = row_cells[1]
                for partner_name, activities in (
                    processed_profile_output.get("raw_data", {})
                    .get("activity", {})
                    .items()
                ):
                    _ = (
                        cell.add_paragraph()
                        .add_run(f"{prettify_name(partner_name)}\n")
                        .bold
                    ) = True
                    self.add_subtable(cell, activities, apply_title_transform=False)

            elif key == "total_wealth_composition":
                cell = row_cells[1]
                for partner_name, twc_dict in (
                    processed_profile_output.get("raw_data", {})
                    .get("total_wealth_composition", {})
                    .items()
                ):
                    cell.add_paragraph().add_run(
                        f"\n{prettify_name(partner_name)}\n"
                    ...
                    for sub_key, name in zip(sub_table_keys, sub_table_names):
                        ...
                        self.add_subtable(
                            cell,
                            twc_dict.get(sub_key, {}).get("assets", []),
                            apply_title_transform=False,
                        )
    ...
    def add_subtable(
        ...
        cell,
        values,
        apply_title_transform: bool = True,
        minimize_first_column: bool = False,
    ):
        if not values:
            return
        ...
        if minimize_first_column:  # siap activities subtable
            original_width = sub_table.columns[0].cells[0].width
            for _cell in sub_table.columns[0].cells:
                _cell.width = 0.15 * cell.width
            for _cell in sub_table.columns[1].cells:
                _cell.width += 0.85 * original_width
        ...
        for idx, elems in enumerate(values, start=1):
            for _cell, (k, v) in zip(sub_table.rows[idx].cells, elems.items()):
                _cell.text = (
                    ...
                    if apply_title_transform
                    else str(v)
                )
                if _cell.text in ["Objections", "SIAP", "Prohibited Relation"]:
                    self.set_cell_color(_cell, "#DC3939")  # red
                elif _cell.text in ["No Objections", "Not SIAP"]:
                    self.set_cell_color(_cell, "#29B95C")  # green
                elif _cell.text in ["Missing Information"]:
                    self.set_cell_color(_cell, "#FFC000")  # yellow-ish
    ...
    def write_word_doc(
        ...
        if edd_profile_output and kyc_profiles_output:
            title = "EDD Assessment/KYC checks"
        elif edd_profile_output:
            title = "EDD Assessment"
        else:
            title = "KYC checks"

        doc.add_heading(
            title + " - Business Relationship " + case_number + "\n",
            level=1,
        )
        ...
        if edd_case:
            ...
            doc.add_paragraph(
                edd_case["contractualpartner_information"]["name"]
                + " - contractual partner,\n"
                + ",\n".join(
                    [
