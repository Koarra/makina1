...
def run_section_activity(
    ...
    if "activity" in kyc_checks_output:
        logger.info("'activity' key exists in kyc_checks_output.")
    else:
        logger.info("'activity' key does NOT exist in kyc_checks_output.")
...
def run_siap_check(partner_info, partner_name: str, kyc_checks_output: dict):
...
def run_section_scap_flag_checks(
    ...
    # Map scap1_flag to "Active" or "Not Active"
    # Update kyc_checks_output with SCAP2 results
    kyc_checks_output["scap_flags"][
        "reason"
    ...
    if edd_parsed:
        ...
        for scap_key, edd_key in scap_mapping.items():
            ...
            if "SCAP" in scap_flag and edd_key not in edd_parsed.get("risk_category", []):
                ...
                kyc_checks_output["scap_flags"][
                    "reason"
                ] += f"* {scap_key} is not reported in EDD risk category.\n"
...
def node_section3_purpose_of_br(state: KycState, llm) -> KycState:
    ...
    return {
        **state,
        "purpose_of_business_relationships": state["kyc_checks_output"][
...
def node_section6_total_assets(state: KycState, llm) -> KycState:
    ...
    return {
        **state,
        "total_assets": state["kyc_checks_output"].get(
            "percentage_total_assets_explained", {}
        ),
...
def node_section7_remarks_total_assets(state: KycState, llm) -> KycState:
    ...
    return {
        **state,
        "remarks_on_total_assets_and_composition": state["kyc_checks_output"].get(
            "remarks_on_total_assets_and_composition", {}
        ),
    }
...
def node_section8_activity(state: KycState, llm) -> KycState:
    ...
    return {**state, "activity": state["kyc_checks_output"]["activity"]}
...
def node_section10_family_situation(state: KycState, llm) -> KycState:
    ...
    return {**state, "family_situation": state["kyc_checks_output"]["family_situation"]}


def node_section11_1_consistency_checks_pep_asm(state: KycState) -> KycState:
    run_section_pep_asm_consistency(
        edd_parsed=state["edd_parsed"],  # patch
        kyc_checks_output=state["kyc_checks_output"],
        pep_sensitivity_present=state["pep_sensitivity_present"],
    )
    return {
        **state,
        "consistency_checks_pep_asm": state["kyc_checks_output"][
            "consistency_checks_pep_asm"
        ],
    }
...
def node_section11_2_consistency_checks_within_kyc(state: KycState, llm) -> KycState:
    ...
    return {
        **state,
        "consistency_checks_within_kyc_contradiction_checks": state[
            "kyc_checks_output"
...
def node_section13_scap_flag_checks(state: KycState) -> KycState:
    # Ensure folder_name and output_folder are passed to run_section_scap_flag_checks
    run_section_scap_flag_checks(
        partner_info=state["partner_info"],
        partner_name=state["partner_name"],
        kyc_checks_output=state["kyc_checks_output"],
        edd_parsed=state["edd_parsed"],  # Pass EDD parsed data
    )

    # Return updated state with SCAP flags
    return {**state, "scap_flags": state["kyc_checks_output"]["scap_flags"]}


def node_section14_siap_checks(state: KycState) -> KycState:
    run_siap_check(
        state["partner_info"], state["partner_name"], state["kyc_checks_output"]
    )

    return {
        **state,
        "siap_checks": state["kyc_checks_output"]["siap_flags"]["raw_data"],
    }
